...
st.set_page_config(page_title="Frontera Eficiencia", page_icon="img/stock-market.png", layout="wide")
theme_plotly = None
...
st.button("Frontera de Eficiencia Modelo Montecarlo para acciones de EEUU", key="inicio", use_container_width=True)
...
if st.button("Ejecutar cálculo de Frontera de Eficiencia", key="glow-on-reg"):
    ...
    with st.expander("Gráfico de Sharpe Ratio de los activos seleccinados"):
      ...
      # Calcular Sharpe Ratio de cada acción individualmente
      sharpe_ratios = {}
      for ticker in tickers:
          ...
          sharpe_ratios[ticker] = (avg_return - rf_rate) / volatility  # Sharpe Ratio

      # Visualizar el gráfico de barras para el Sharpe Ratio de cada acción
      fig, ax = plt.subplots(figsize=(10, 6))
      ax.bar(sharpe_ratios.keys(), sharpe_ratios.values(), color='#9333ff')
      ax.set_xlabel('Acciones/Tickers')
      ax.set_ylabel('Sharpe Ratio')
      ax.set_title('Sharpe Ratio de cada acción en el Portafolio')
      st.pyplot(fig)  
